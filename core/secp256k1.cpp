...
void mul_wide_256(const BigInt& a, const BigInt& b, std::uint64_t out[8]) {
    // Implementação C++ otimizada usando __int128 para performance portátil.
    // Realiza multiplicação 256x256 -> 512 com propagação de carry.
...
// to_bytes32 is now inline in secp256k1.hpp
...
bool parse_big_int(const std::string& text, BigInt& out) {
    ...
    if (value.size() > 2u && value[0] == '0' && (value[1] == 'x' || value[1] == 'X')) {
        ...
        // Heuristic: If it looks like a hex string (even without 0x prefix), treat as hex.
        // For addresses and keys, we often get hex strings without prefix.
        if (value.size() >= 32) {
             base = 16;
        } else {
            bool has_hex = false;
