...
void hash4_avx(const std::uint8_t* const data[4], std::size_t length, std::uint8_t* const out[4]) {
    if (!data || !out) return;
    ...
    // AVX 128-bit: use 256-bit registers but only process 4x32-bit = 128 bits
    // Same approach as SSE4 but using AVX intrinsics
    ...
    #define LOAD_W4(i) _mm256_set_epi32( \
        *(const std::uint32_t*)(data[3] + i*4), *(const std::uint32_t*)(data[2] + i*4), \
        *(const std::uint32_t*)(data[1] + i*4), *(const std::uint32_t*)(data[0] + i*4), \
        *(const std::uint32_t*)(data[3] + i*4), *(const std::uint32_t*)(data[2] + i*4), \
        *(const std::uint32_t*)(data[1] + i*4), *(const std::uint32_t*)(data[0] + i*4))
    
    #define ROTR4(x, n) _mm256_or_si256(_mm256_srli_epi32(x, n), _mm256_slli_epi32(x, 32 - n))
    #define SHR4(x, n) _mm256_srli_epi32(x, n)
    #define SIG0_4(x) _mm256_xor_si256(ROTR4(x, 7), _mm256_xor_si256(ROTR4(x, 18), SHR4(x, 3)))
    #define SIG1_4(x) _mm256_xor_si256(ROTR4(x, 17), _mm256_xor_si256(ROTR4(x, 19), SHR4(x, 10)))
    #define EP0_4(x) _mm256_xor_si256(ROTR4(x, 2), _mm256_xor_si256(ROTR4(x, 13), ROTR4(x, 22)))
    #define EP1_4(x) _mm256_xor_si256(ROTR4(x, 6), _mm256_xor_si256(ROTR4(x, 11), ROTR4(x, 25)))
    #define CH4(e, f, g) _mm256_xor_si256(_mm256_and_si256(e, f), _mm256_andnot_si256(e, g))
    #define MAJ4(a, b, c) _mm256_or_si256(_mm256_and_si256(a, b), \
        _mm256_or_si256(_mm256_and_si256(a, c), _mm256_and_si256(b, c)))
    
    auto expand_schedule4 = [&]() {
        for (int i = 16; i < 64; ++i) {
            W[i] = _mm256_add_epi32(
                _mm256_add_epi32(SIG1_4(W[i - 2]), W[i - 7]),
                _mm256_add_epi32(SIG0_4(W[i - 15]), W[i - 16])
            );
        }
    };
    
    auto run_block4 = [&](__m256i state[8]) {
        ...
        for (int i = 0; i < 64; ++i) {
            __m256i T1 = _mm256_add_epi32(
                _mm256_add_epi32(_mm256_add_epi32(H, EP1_4(E)), CH4(E, F, G)),
                _mm256_add_epi32(_mm256_set1_epi32(kTable[i]), W[i])
            );
            __m256i T2 = _mm256_add_epi32(EP0_4(A), MAJ4(A, B, C));
    ...
    if (length == 33) {
        for (int i = 0; i < 8; ++i) W[i] = _mm256_shuffle_epi8(LOAD_W4(i), bswap_mask);
        W[8] = _mm256_set_epi32(((uint32_t)data[3][32]<<24)|0x800000, ((uint32_t)data[2][32]<<24)|0x800000,
                             ((uint32_t)data[1][32]<<24)|0x800000, ((uint32_t)data[0][32]<<24)|0x800000,
                             ((uint32_t)data[3][32]<<24)|0x800000, ((uint32_t)data[2][32]<<24)|0x800000,
                             ((uint32_t)data[1][32]<<24)|0x800000, ((uint32_t)data[0][32]<<24)|0x800000);
        for (int i = 9; i < 15; ++i) W[i] = zero;
        W[15] = _mm256_set1_epi32(kSingleBlockBits);
        expand_schedule4(); run_block4(state);
    } else {
        for (int i = 0; i < 16; ++i) W[i] = _mm256_shuffle_epi8(LOAD_W4(i), bswap_mask);
        expand_schedule4(); run_block4(state);
        W[0] = _mm256_set_epi32(((uint32_t)data[3][64]<<24)|0x800000, ((uint32_t)data[2][64]<<24)|0x800000,
                             ((uint32_t)data[1][64]<<24)|0x800000, ((uint32_t)data[0][64]<<24)|0x800000,
                             ((uint32_t)data[3][64]<<24)|0x800000, ((uint32_t)data[2][64]<<24)|0x800000,
                             ((uint32_t)data[1][64]<<24)|0x800000, ((uint32_t)data[0][64]<<24)|0x800000);
    ...
    // Store results - extract lower 128 bits from 256-bit registers (duplicated values)
    #define STORE_H4(i, reg) do { \
        __m256i swapped = _mm256_shuffle_epi8(reg, bswap_mask); \
        std::uint32_t arr[8]; _mm256_storeu_si256((__m256i*)arr, swapped); \
