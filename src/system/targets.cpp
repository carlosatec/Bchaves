...
bool validate_address_payload(const std::vector<std::uint8_t>& decoded) {
    ...
    qchaves::core::ByteVector body(decoded.begin(), decoded.begin() + 21);
    const auto digest = qchaves::core::double_sha256(body);
...
TargetType detect_type(const std::string& line) {
    if (line.size() == 66u && qchaves::core::is_hex(line) && (line.rfind("02", 0) == 0 || line.rfind("03", 0) == 0)) {
        return TargetType::pubkey_compress;
    }
    if (line.size() == 130u && qchaves::core::is_hex(line) && line.rfind("04", 0) == 0) {
        return TargetType::pubkey_uncompress;
    }
    if (line.size() >= 26u && line.size() <= 35u && qchaves::core::is_base58_string(line)) {
...
TargetLoadResult load_target_inline(const std::string& input, bool require_pubkeys_only) {
    ...
    if (type == TargetType::address_btc) {
        bool ok = false;
        const auto decoded = qchaves::core::base58_decode(cleaned, &ok);
        ...
        entry.payload = qchaves::core::from_hex(cleaned);
    }
