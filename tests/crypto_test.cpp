...
// ============================================================
static BigInt get_pseudo_random_scalar(uint32_t seed) {
    BigInt r;
    uint64_t state = seed;
    for (int i = 0; i < 4; ++i) {
    ...
    return r % kCurveOrder;
}

static void test_curve_ops() {
    std::cout << "[*] test_curve_ops\n";

    BigInt one(1);
    Secp256k1Point G = secp256k1_multiply(one);
    Secp256k1Point twoG = secp256k1_multiply(BigInt(2));
    Secp256k1Point threeG = secp256k1_multiply(BigInt(3));

    Secp256k1Point add_g_g = secp256k1_add(G, G);
    EXPECT(add_g_g.x == twoG.x && add_g_g.y == twoG.y, "G + G == 2G");

    Secp256k1Point add_2g_g = secp256k1_add(twoG, G);
    EXPECT(add_2g_g.x == threeG.x && add_2g_g.y == threeG.y, "2G + G == 3G");

    Secp256k1Point add_g_2g = secp256k1_add(G, twoG);
    EXPECT(add_g_2g.x == threeG.x && add_g_2g.y == threeG.y, "G + 2G == 3G");

    Secp256k1Point inf;
    Secp256k1Point add_g_inf = secp256k1_add(G, inf);
    EXPECT(add_g_inf.x == G.x && add_g_inf.y == G.y && !add_g_inf.infinity, "G + inf == G");
    Secp256k1Point add_inf_g = secp256k1_add(inf, G);
    EXPECT(add_inf_g.x == G.x && add_inf_g.y == G.y && !add_inf_g.infinity, "inf + G == G");

    Secp256k1Point negG = G;
    negG.y = mod_sub(BigInt(0), G.y, kFieldPrime);
    Secp256k1Point add_g_negg = secp256k1_add(G, negG);
    EXPECT(add_g_negg.infinity, "G + (-G) == infinity");
}

static void test_endomorphism_relation() {
    std::cout << "[*] test_endomorphism_relation\n";

    BigInt one(1);
    Secp256k1Point G = secp256k1_multiply(one);
    
    Secp256k1Point phi_G = phi(G);
    Secp256k1Point lambda_G = secp256k1_multiply(kGLV_Lambda);
    EXPECT(phi_G.x == lambda_G.x && phi_G.y == lambda_G.y, "phi(G) == lambda * G");

    Secp256k1Point fiveG = secp256k1_multiply(BigInt(5));
    Secp256k1Point phi_fiveG = phi(fiveG);
    BigInt five_lambda = mod_mul(BigInt(5), kGLV_Lambda, kCurveOrder);
    Secp256k1Point expected = secp256k1_multiply(five_lambda);
    EXPECT(phi_fiveG.x == expected.x && phi_fiveG.y == expected.y, "phi(5G) == (5 * lambda) * G");
}
...
static void test_glv_decomposition() {
    ...
    bool all_algebra_ok = true;
    for (int i = 0; i < 10000; ++i) {
        BigInt k = get_pseudo_random_scalar(i + 987654);
        BigInt k1, k2;
        bool k1_neg, k2_neg;
        decompose_glv(k, k1, k2, k1_neg, k2_neg);
        
        if (k1.limbs[2] != 0 || k1.limbs[3] != 0 || k2.limbs[2] != 0 || k2.limbs[3] != 0) {
            all_bounds_ok = false;
        }
        
        BigInt term2 = mod_mul(k2, kGLV_Lambda, kCurveOrder);
        BigInt lhs;
        if (k1_neg) {
            lhs = mod_sub(kCurveOrder, k1, kCurveOrder);
        } else {
            lhs = k1 % kCurveOrder;
        }
        
        if (k2_neg) {
            lhs = mod_sub(lhs, term2, kCurveOrder);
        } else {
            lhs = mod_add(lhs, term2, kCurveOrder);
        }
        
        if (lhs != (k % kCurveOrder)) {
            all_algebra_ok = false;
        }
    }
    EXPECT(all_bounds_ok, "All 10,000 decompositions are strictly bounded by 2^128");
    EXPECT(all_algebra_ok, "All 10,000 decompositions satisfy k1 + lambda*k2 == k (mod n)");

    bool all_mult_ok = true;
    for (int i = 0; i < 200; ++i) {
        BigInt k = get_pseudo_random_scalar(i + 12345);
        Secp256k1Point p1 = secp256k1_multiply(k);
        Secp256k1Point p2 = secp256k1_multiply_glv(k);
        if (p1.x != p2.x || p1.y != p2.y || p1.infinity != p2.infinity) {
            all_mult_ok = false;
        }
    }
    EXPECT(all_mult_ok, "GLV multiplication matches direct multiplication for 200 random scalars");
}
