...
### Fase 13: Otimização Estrutural e Correção de Checkpoint Híbrido (DONE)
- [x] **Address Hybrid Integrity**: Implementar array de rastreamento de chunks (low-water mark) para garantir 100% de precisão na retomada do checkpoint.
- [x] **Kangaroo State Sync**: Adicionar sincronização obrigatória da frota (`local_fleet`) no encerramento das threads para evitar perda e assincronia de telemetria após um SIGINT.
- [x] **Address Engine SIMD Pipeline**: Consolidar inversões de endianness em header no Core (`simd_hashing.hpp`) via shuffle vetorial com *fallback* cross-platform (AVX2/SSSE3/NEON/Escalar) e forçar o pipeline hash 8-way.
- [x] **Kangaroo Queue DPs & Thread-Safety**: Corrigir vulnerabilidade de *Data Race* no `AdaptiveCuckooFilter` e evitar *Deadlocks* separando os locks (`filter_mtx` e `shard.mtx`) processados a partir de Buffers Locais por thread.
- [x] **Validation via WSL**: Validado via suite de testes rodando no WSL.

**Plans:** 1 plan
- [x] [13-01-PLAN.md](phases/13-hybrid-kangaroo-optimization/13-01-PLAN.md) — Hybrid & Kangaroo Optimization Plan.

**Review:** [13-REVIEW.md](phases/13-hybrid-kangaroo-optimization/13-REVIEW.md)

---

### Fase 14: Precisão Criptográfica e Hardening GLV (DONE)
- [x] **GLV Lambda Correction**: Corrigir constante `kGLV_Lambda` (eigenvalue do endomorfismo) na implementação de secp256k1.
- [x] **GLV Rounding Arithmetic**: Implementar divisão longa de 512 bits e multiplicação de $256 \times 128$ bits para o Babai Rounding exato, com as constantes de base corrigidas.
- [x] **Deduplicação de Símbolos**: Unificar `to_lower` removendo a cópia redundante em `core/secp256k1`.
- [x] **Suíte de Testes Criptográficos**: Criar `tests/crypto_test.cpp` para validar as operações da curva e o endomorfismo GLV sob estresse.

**Planos:** 1 plano
- [x] [14-01-PLAN.md](phases/14-crypto-precision-glv-hardening/14-01-PLAN.md) — Precisão Criptográfica e Hardening GLV.
