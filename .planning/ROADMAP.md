...
### Fase 11: Resiliência e Otimização Avançada
- [ ] **Checkpoint Integrity**: Implementar soma de verificação (CRC32) para validar a integridade dos arquivos `.ckp`.
- [ ] **NUMA-Aware Allocation**: Otimizar alocação de memória para processadores multi-socket (afinidade de nó).
- [ ] **Manual Prefetching**: Integrar `_mm_prefetch` nos loops críticos do Cuckoo Filter.
- [ ] **Structure Refactoring**: Renomear `core/address.cpp` para `core/bitcoin_format.cpp` e centralizar telemetria.
- [ ] **Unit Testing Expansion**: Adicionar testes de estresse para condições de corrida na troca de contexto de sinal.
