...
### Fase 13: Otimização Estrutural e Correção de Checkpoint Híbrido (PLANNED)
- [ ] **Address Hybrid Integrity**: Implementar array de rastreamento de chunks (low-water mark) para garantir 100% de precisão na retomada do checkpoint.
- [ ] **Kangaroo State Sync**: Adicionar sincronização obrigatória da frota (`local_fleet`) no encerramento das threads para evitar perda e assincronia de telemetria após um SIGINT.
- [ ] **Address Engine SIMD Pipeline**: Consolidar inversões de endianness em header no Core (`simd_hashing.hpp`) via shuffle vetorial com *fallback* cross-platform (AVX2/SSSE3/NEON/Escalar) e forçar o pipeline hash 8-way.
- [ ] **Kangaroo Queue DPs & Thread-Safety**: Corrigir vulnerabilidade de *Data Race* no `AdaptiveCuckooFilter` e evitar *Deadlocks* separando os locks (`filter_mtx` e `shard.mtx`) processados a partir de Buffers Locais por thread.

**Plans:** 1 plan
- [ ] [13-01-PLAN.md](phases/13-hybrid-kangaroo-optimization/13-01-PLAN.md) — Hybrid & Kangaroo Optimization Plan.
